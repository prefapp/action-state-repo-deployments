...
const { createBranch, uploadToRepo, getCurrentCommit, createComment, mergePr, createPr } = require('./git')
...
class Deployment {
    ...
    _template(_config) {
        throw new Error(`Method 'template' must be implemented in kind ${this.kind}`)
    }
    ...
    _postTemplate() {
        ...
        outputFiles.forEach(file => {
            ...
            for (const doc of yamlContent) {
                ...
                if (!crKind || !crName) {
                    throw new Error(`File ${file} does not kind or metadata.name. Got ${crKind} and ${crName}`)
                }

                const newFileName = `${crKind}.${crName}.yml`;
                const newFilePath = path.join(this.config.outputDir, this.kind, ...this.folders, newFileName);
    ...
    _isAutoMerge() {
        ...
        const autoMergeFile = path.join(this.config.deploymentsDir, this.kind, ...this.folders, this.config.environment, 'AUTO_MERGE')
    ...
    async _postVerify() {
        ...
        await createBranch(octo, owner, repo, `refs/heads/${this.config.environment}-${this.kind}-${this.folders.join('-')}`, commit.commitSha)

        // Add a commit with the changes
        await uploadToRepo(octo, path.join(this.config.outputDir), owner, repo, `${this.config.environment}-${this.kind}-${this.folders.join('-')}`)
        ...
        const pr = prs.data.find(pr => pr.head.ref === `${this.config.environment}-${this.kind}-${this.folders.join('-')}`)
        ...
        if (!pr) {
            ...
            await createComment(octo, owner, repo, newPrNumber, `Original PR: #${this.config.prNumber}`)
    ...
    _verify() {
        ...
        files.forEach(file => {
            ...
            if (data?.metadata?.namespace !== namespace) {
                throw new Error(`File ${file} does not have the correct metadata.namespace. Expected ${this.tenant}-${this.app}-${this.config.environment} and got data ${data?.metadata?.name}`)
            }
        })
...
module.exports = { createDeployment, Deployment, AppDeployment, SysServiceDeployment }
